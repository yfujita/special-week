...
TARGET_KAISAI_DATES: Final[List[str]] = [
    ...
    # --- 100レース化に向けた母数拡充（2024〜2025の重賞開催日を追加） ---
    # 既出の開催日は重複してもrace_id単位で除外されるため安全。多重賞の週末を中心に、
    # ダート重賞（武蔵野S・みやこS・チャンピオンズC）と各競馬場・距離が分散するよう選定。
    '20240114',  # 日経新春杯(G2)/京成杯(G3)
    '20240310',  # 金鯱賞(G2)/中山牝馬S(G3)/フィリーズレビュー(G2)
    '20240421',  # 皐月賞(G1,芝2000,中山)
    '20240505',  # NHKマイルC(G1)/新潟大賞典(G3)
    '20240512',  # ヴィクトリアマイル(G1)/京王杯SC(G2)
    '20240602',  # 安田記念(G1,芝1600,東京)
    '20240616',  # エプソムC(G3)/函館スプリントS(G3)
    '20241006',  # 毎日王冠(G2)/京都大賞典(G2)
    '20241013',  # 秋華賞(G1,芝2000,京都)
    '20241103',  # アルゼンチン共和国杯(G2)/みやこS(G3,ダ)/ファンタジーS(G3)
    '20241110',  # エリザベス女王杯(G1)/武蔵野S(G3,ダ)/福島記念(G3)
    '20241117',  # マイルCS(G1)/東スポ杯2歳S(G2)
    '20241215',  # 朝日杯FS(G1)/ターコイズS(G3)
    '20241228',  # ホープフルS(G1,芝2000,中山)
    '20250112',  # 日経新春杯(G2)/京成杯(G3)
    '20250309',  # 金鯱賞(G2)/中山牝馬S(G3)/フィリーズレビュー(G2)
    '20250420',  # 皐月賞(G1,芝2000,中山)
    '20250504',  # NHKマイルC(G1)/新潟大賞典(G3)
    '20250511',  # ヴィクトリアマイル(G1)/京王杯SC(G2)
    '20250601',  # 安田記念(G1,芝1600,東京)
    '20250629',  # 宝塚記念(G1,芝2200,京都)
    '20251102',  # 天皇賞(秋)(G1,芝2000,東京)
    '20251123',  # ジャパンC(G1,芝2400,東京)
    '20251207',  # チャンピオンズC(G1,ダ1800,中京)
    '20251228',  # 有馬記念(G1)/ホープフルS(G1)
    # 100レース到達のための追加分（複数重賞のある週末を中心に）
    '20240204',  # きさらぎ賞(G3)/東京新聞杯(G3)
    '20240218',  # フェブラリーS(G1,ダ)/京都記念(G2)/小倉大賞典(G3)
    '20240317',  # スプリングS(G2)/阪神大賞典(G2)/フラワーC(G3)
    '20240407',  # 桜花賞(G1)/阪神牝馬S(G2)
    '20250216',  # 京都記念(G2)/クイーンC(G3)/ダイヤモンドS(G3)
    '20251019',  # 菊花賞(G1)/府中牝馬S(G2)
]
